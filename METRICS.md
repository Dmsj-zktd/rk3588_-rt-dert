...
## 任务8 回退记录（2026-08-16，按用户指令回退）

- 任务8 内容：显示帧按帧号重排序（DisplayReorderer，输出严格递增、前向跳帧、迟到帧丢弃）+ 66ms 显示节奏 + 显示队列 4→12；
  V4L2 帧释放后立即 QBUF、相机输入队列满实时丢帧、poll 500→1500ms、退出期信号中断不告警。
- 任务8 实测：视频三素材 / 相机三尺寸 **backwards 全 0**；相机 640x480 15.66 FPS、640x640 15.54、720p 10.81（源上限）；
  "no frame" 警告全 0；显示延迟 ≤642ms；单测 30/30。
- 用户反馈：-p 2 -n 14 -P 3 长视频（test_people_small_little.mp4）CPU 55%→88%、720p 每 30 帧间隔 2s→3s。
- 复核（未复现）：同配置非显示 FPS 15.82~15.97、CPU 稳定 403~439%（≈54%/核）、30 帧≈1.9s、RSS 稳定 1.65GB；
  显示态 CPU 422~464%、Xorg 1~3%、backwards=0。仍按用户决定回退。
- 回退执行：任务8 源码备份至本地 `cpp_task08/`；git 分支 `task8-backup`（beb4a05）保留历史；
  master 重置回 `f2bd68c` 并重新推送；板端同步回退版本。

## 任务9：-F/--fps 输入/输出帧率覆盖（2026-08-16）

- 需求：未指定 `-F/--fps` 时输入/输出 fps 保持现有默认；指定后视频文件与 camera 的输入/输出 fps 均按设定值。
- 实现（main.cc）：视频文件模式按 -F 值对喂帧做 sleep 节拍限速 + 输出 writer fps 覆盖；camera 模式采集请求 + 输出 fps 覆盖；-F 指定时跳过 VFR 两遍法 probe。
- 验证（performance，-p 2 -n 8 -P 3）：

| 场景 | Overall FPS | 输出容器 fps（ffprobe） |
|------|-----------|------------------------|
| cars_2s.mp4 无 -F | 13.18 | **30/1**（源） |
| cars_2s.mp4 -F 10 | **9.68**（输入限速） | **10/1** |
| cars_2s.mp4 -F 30 | 13.23（流水线瓶颈） | **30/1** |
| camera 720p -F 30 | 10.78（源上限） | **30/1** |
| camera 720p -F 15 | 10.77（源上限） | **15/1** |

- 回归：无 -F 视频基线 cars.mp4 n8 15.23 FPS / pre 4.65ms / npu 514ms / post 0.51ms，与修改前一致；单测 29/29。
