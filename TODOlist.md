...
| 8 | **--display 抖动/相机停流修复（任务8，实验性）** | **已回退（恢复任务7状态）** | 2026-08-16 | 往返抖动修复有效，但用户反馈 720p 播放变卡/长视频 CPU 上升（复核未复现）；按用户指令回退到任务8前（f2bd68c）；源码备份于 cpp_task08/，历史保留于 git 分支 task8-backup |
| 9 | **-F/--fps 输入/输出帧率覆盖（视频文件 + camera）** | **已完成并上传入库** | 2026-08-16 | 指定 -F 后：视频文件输入限速+输出fps、camera 采集+输出fps 均按设定值；未指定保持原默认；无基准性能影响；commit 505c0eb |
...
### 2026-08-16 任务8 回退（按用户指令）

- 任务8（显示重排序防往返抖动 + V4L2 立即归还/相机实时丢帧/poll 放宽）曾提交并推送（beb4a05、66aa913）。
- 用户反馈：往返抖动已消除，但 1280x720 播放变卡（每 30 帧间隔 2s→3s+）、密集长视频（test_people_small_little.mp4）CPU 由 ~55% 升至 88%+（-p 2 -n 14 -P 3）。
- 复核：按 -p 2 -n 14 -P 3 复测长视频，非显示 FPS 15.82~15.97、CPU 稳定 403~439%（≈54%/核）、30 帧≈1.9s；显示态 CPU 422~464%、Xorg 仅 1~3%、backwards=0；相机 720p 10.8fps 为 YUYV 源上限——未复现 88%/3s。用户仍决定回退。
- 执行回退：任务8 源码完整备份至本地 workspace `cpp_task08/`（36+1 文件，对应 beb4a05 状态）；
  git 分支 `task8-backup` 保留任务8 历史；master 重置回 `f2bd68c`（任务7 最新版）并重新推送。

### 2026-08-16 任务9 完成（-F/--fps 输入输出帧率覆盖）

- 需求：未指定 `-F/--fps` 时输入/输出 fps 保持现有默认；指定后视频文件与 camera 的输入/输出 fps 均按设定值。
- 实现：视频文件模式新增输入喂帧限速（按 -F 值 sleep 节拍）+ 输出 writer fps 覆盖；camera 模式沿用采集请求 + 输出 fps 覆盖；`-F` 指定时跳过 VFR 两遍法 probe。
- 验证：cars_2s 无 -F 输出 30fps（源）；-F 10 输入限速 9.68fps、输出 10fps；-F 30 输出 30fps、流水线 13.2fps；
  camera -F 15/-F 30 输出 15/30fps（720p 源上限 10.8fps 不变）。
- 回归：无 -F 视频基线 cars.mp4 n8 15.23 FPS / post 0.51ms，与修改前一致（单测 29/29）。
- 上传入库（2026-08-16）：用户同意，README 已更新（-F/--fps 用法），commit `505c0eb` 已推送 gitee。
