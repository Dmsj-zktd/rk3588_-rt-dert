...
| 8 | **--display 实时播放抖动/相机停流修复（抽帧/往返抖动 + no frame 警告）** | **已完成（待用户确认上传）** | 2026-08-16 | 显示按帧号重排序（backwards=0）+ 15fps 节奏 + V4L2 帧立即归还 + 相机实时丢帧 + poll 1500ms；视频/相机均无往返抖动，相机三尺寸 0 警告；单测 30/30 |
...
### 2026-08-16 任务8 完成（显示抖动/相机停流修复）

- 根因 1（往返抖动）：`-P 3` 后处理完成顺序与帧号不一致，显示队列按完成顺序直接播放 → 画面倒退/往返。
  修复：`DisplayReorderer` 按帧号重排序（严格递增输出），前序缺失超时/超容时前向跳帧，迟到旧帧直接丢弃；显示节奏 66ms（~15fps）均匀推进；显示队列容量 4→12 吸收完成突发。
- 根因 2（相机 "no frame" 警告/卡顿）：V4L2 buffer 等到下次 read_frame 才归还，主线程阻塞在满队列时相机 4 个 buffer 耗尽停流。
  修复：帧释放后立即 VIDIOC_QBUF（worker 并发归还）；相机输入队列满时实时丢帧（dropped=0 实测，保底策略）；poll 超时 500→1500ms 兼容 UVC 启动延迟；退出期信号中断不再告警。
- 实测（--display，performance）：视频 cars.mp4 15.19~15.27 FPS / skips 4-8 / lag 345-546ms；cars-from uav VFR 15.68 / skips 16 / lag 583ms；cars4 15.67 / skips 23 / lag 569ms —— 三视频 **backwards=0**。
- 实测（相机 --display）：640x480 15.66 FPS / **WARN=0** / lag 642ms；640x640 15.54 / **WARN=0** / lag 623ms；1280x720 10.81 / **WARN=0** / lag 4ms —— 三尺寸 **backwards=0**。
- 回归：单测 **30/30**（新增 DisplayReorderer 测试）；视频无显示 n8 15.35 FPS / post 450us 无回退。
